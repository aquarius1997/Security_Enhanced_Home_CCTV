...
JNIEXPORT jstring JNICALL Java_IBSClient_AES_1GCM_1enc(JNIEnv *env, jobject obj, jstring Plaintext, jint textlen, jstring SessionKey, jstring RandIV)
{
	...
	IV = (unsigned char *)(*env)->GetStringUTFChars(env, RandIV, NULL);
	plaintext = (*env)->GetStringUTFChars(env, Plaintext, NULL);
	sessionkey = hexstr_to_char((*env)->GetStringUTFChars(env, SessionKey, NULL)); //
...
///////
JNIEXPORT jstring JNICALL Java_IBSClient_AES_1GCM_1dec(JNIEnv *env, jobject obj, jstring Ciphertext, jint Textlen, jstring key, jstring RandIV)
{
	unsigned char *plaintext;
	unsigned char *ciphertext;
	...
	IV = (unsigned char *)(*env)->GetStringUTFChars(env, RandIV, NULL);
	tempstr = (unsigned char *)(*env)->GetStringUTFChars(env, Ciphertext, NULL);
	plaintext = (unsigned char *)calloc(textlen, sizeof(unsigned char));
	ciphertext = bit2char(tempstr, Textlen);

	keytext = hexstr_to_char((*env)->GetStringUTFChars(env, key, NULL));

    KeyExpansion(keytext, (unsigned long *)ekey, 128);//? ???
	...
	return (*env)->NewStringUTF(env, (const char*)plaintext);
}
