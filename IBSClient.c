...
JNIEXPORT jstring JNICALL Java_IBSClient_GetIV(JNIEnv *env, jobject obj)
{
    unsigned char IV[4];
   
    RAND_bytes(IV, 4);
        
    return (*env)->NewStringUTF(env,unsign2sign(IV,4));
}
...
JNIEXPORT jstring JNICALL Java_IBSClient_getGMod(JNIEnv *env, jobject obj, jstring exp){
	BIGNUM
		*r = BN_new(),
		*n = BN_new(),
		*g = BN_new(),
		...
		*c_sqrd_sk = BN_new(),
		*g_sqrd_exp = BN_new(),
		*base_sqrd_exp = BN_new(),
		*exponent = BN_new();
	unsigned char *bin_exp;
	unsigned int exp_len=0;

	BN_CTX* ctx;
	const char *expo = (*env)->GetStringUTFChars(env, exp, NULL);
	BN_hex2bn(&g, info_g);
	BN_hex2bn(&n, info_n);
	BN_hex2bn(&exponent, expo);

	ctx = BN_CTX_new();
	BN_CTX_init(ctx);

	BN_mod_exp(g_sqrd_exp, g, exponent, n, ctx);//g^r

	exp_len = BN_num_bytes(g_sqrd_exp);
	bin_exp = (unsigned char*)calloc(exp_len, sizeof(unsigned char));
	BN_bn2bin(g_sqrd_exp, bin_exp);
	return (*env)->NewStringUTF(env, unsign2sign(bin_exp, exp_len));

}

JNIEXPORT jstring JNICALL Java_IBSClient_getBaseExp(JNIEnv *env, jobject obj, jstring base, jstring exp){
	BIGNUM
		*r = BN_new(),
		*n = BN_new(),
		*g = BN_new(),
	...
	BN_CTX* ctx;
	const char *expo = (*env)->GetStringUTFChars(env, exp, NULL);
	const char *base_char = (*env)->GetStringUTFChars(env, base, NULL);
	BN_hex2bn(&g, info_g);
	BN_hex2bn(&n, info_n);
	BN_hex2bn(&exponent, expo);
	...
	BN_hex2bn(&bignum_base, base_char);
	BN_mod_exp(base_sqrd_exp, bignum_base, exponent, n, ctx);
	exp_len = BN_num_bytes(base_sqrd_exp);
	bin_exp = (unsigned char*)calloc(exp_len, sizeof(unsigned char));
	BN_bn2bin(base_sqrd_exp, bin_exp);
	return (*env)->NewStringUTF(env, unsign2sign(bin_exp, exp_len));
